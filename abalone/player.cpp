...
duration_t total_duration{0}; // initialize to zero
unsigned nMoves = 0; // number of moves drawn
...
void MyDomain::received(char* str)
{
	...
	if(state == Board::win1 ||
	   state == Board::win2 ||
	   state == Board::timeout1 ||
	   state == Board::timeout2) {
		printf("Total time for %u moves for player %s: %.3f secs\n",
		       nMoves, (myColor == Board::color1) ? "O":"X",
		       total_duration.count() / 1000.0);	
		}
		printf("Average time for %u moves for player %s: %.3f secs\n",
		       nMoves, (myColor == Board::color1) ? "O":"X",
		       total_duration.count() / 1000.0 / nMoves);
	...
	total_duration += std::chrono::duration_cast<duration_t>(end - start);
	gettimeofday(&t2,0);
